...
cykelvag_fil <- "G:/skript/henrik/GitHub/Region-Atlas/QGIS/cykelvagar.gpkg"

cykelvagar <- st_read(cykelvag_fil, crs = 3006)

#mapview(cykelvagar)

nvdb_fil <- "G:/skript/henrik/GitHub/Region-Atlas/QGIS/nvdb_20_buff30km.gpkg"

nvdb <- st_read(nvdb_fil, crs = 3006)

nvdb_reg_atlas <- nvdb %>% 
  rename(gatunamn = Gatunamn_Namn,
         hastighetsgrans = Hastighetsgrans_HogstaTillatnaHastighet_F,
         vagtrafiknattyp = Vagtrafiknat_Vagtrafiknattyp
         )
nvdb_reg_atlas <- nvdb_reg_atlas %>% 
  as.character(nvdb_reg_atlas$hastighetsgrans)
  
  
glimpse(nvdb_reg_atlas)

mapview(nvdb_reg_atlas, burst = "hastighetsgrans")

mapview(nvdb_reg_atlas, lwd = 5, zcol = "hastighetsgräns")
```
