---
title: "Region-Atlas, Kollektivtrafik"
author: ""
date: "Uppdaterad den `r Sys.Date()`"
...
stadstrafik_fil <- "G:/skript/henrik/GitHub/Region-Atlas/QGIS/gtfs_stadstrafik.gpkg"

stadstrafik <- st_read(stadstrafik_fil, crs = 3006)

landsbygdstrafik_fil <- "G:/skript/henrik/GitHub/Region-Atlas/QGIS/gtfs_landsbyggdstrafik.gpkg"

landsbygdstrafik <- st_read(landsbygdstrafik_fil, crs = 3006)

hundrafemtiolinjer_fil <- "G:/skript/henrik/GitHub/Region-Atlas/QGIS/gtfs_hundrafemtiolinjer.gpkg"

hundrafemtiolinjer <- st_read(hundrafemtiolinjer_fil, crs = 3006)

#=====================KTF fastigheter

ktf_punkter_fil <- "G:/skript/henrik/GitHub/Region-Atlas/QGIS/ktf_fastighet_punkt.gpkg"

ktf_punkter <- st_read(ktf_punkter_fil, crs = 3006)

ktf_byggnader_fil <- "G:/skript/henrik/GitHub/Region-Atlas/QGIS/ktf_fastighet_byggnad.gpkg"

ktf_byggnader <- st_read(ktf_byggnader_fil, crs = 3006)

ktf_yta_fil <- "G:/skript/henrik/GitHub/Region-Atlas/QGIS/ktf_fastighet_yta.gpkg"

ktf_yta <- st_read(ktf_yta_fil, crs = 3006)
...
mapview(hallplatslagen, zcol = "busslinjer", legend = FALSE, col.regions = "darkgreen", cex = 1.5, layer.name = c("Hållplatslägen"))+
  mapview(centrumkoordinat_hallplatser, col.regions = "green", zcol = "stop_name", legend =FALSE, cex = 2.5, layer.name = c("Centrumkoordinat"))+
  mapview(tatorter, col.regions = "gainsboro",legend = FALSE, alpha.regions = 0.1, label = "tatort", layer.name = c("Tätorter"))+
  ...
  mapview(jarnvagstation, col.regions = "white", color = "black", cex = 5, legend = FALSE)+
  mapview(jarnvag, color = "grey29", lwd = 2, legend = FALSE)+
  mapview(hundrafemtiolinjer, color = "green1", lwd = 5, legend = FALSE, label = "busslinje")+
  mapview(landsbygdstrafik, color = "green3", lwd = 3, legend = FALSE, label = "busslinje")+
  mapview(stadstrafik, color = "green4", lwd = 1, legend = FALSE, label = "busslinje")+
  mapview(ktf_punkter, col.regions = "khaki", legend = FALSE, label = "Populärnamn", layer.name = c("KTF fastigheter"))+
  mapview(ktf_yta, col.regions = "khaki", legend = FALSE, label = "fastighet")+
  mapview(ktf_byggnader, col.regions = "khaki", legend = FALSE, label = "andamal1")
...
<br>
<details>
  <summary>Information och instruktioner</summary>
<div title="Om kartan"> Kartan är en interaktiv visualisering av data relevant för KTF i Dalarna. Innehållet kommer att byggas på efterhand som kunskap ökar och data förändras. Data på kollektivtrafiken laddas ner från Trafiklab via en API. Kartan är interaktiv med lager som kan tändas och släckas med ikonen under zoom uppe till vänster i kartan. Ett vänsterklick på ett objekt visar all tillgänglig information i den tabell som ligger till grund för aktuellt lager (ett vänsterklick utanför popupen i kartan stänger tabellen). När muspekaren vilar på ett objekt visas en förinställd variabel beroende på vilket lager som är tänt. T.ex muspekare på tjock guldig linje visar vilka turer som trafikerar sträckan mellan tätorts-centroider i gult. Gröna cirklar visar antal avgångar med storleken på cirkeln och namn på aktuell hållplats, närliggande mörkare gröna cirklar visar hållplatslägen och vilka busslinjer som trafikerar aktuell hållplats>.
