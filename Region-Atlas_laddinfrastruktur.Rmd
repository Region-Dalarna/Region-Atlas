...
laddstationer_anslut <- laddstationer %>% 
  select(id, namn, gata, gatnr, postnr, ort, kom_kod, lan,
         lan_kod, kommun, lages_bskrvng, agare, operator,  n_ladd_punkt, kommentar, 
         kontakt, skapad, uppdaterad, station_status) %>% 
  filter(lan_kod == '20') %>% 
  group_by(kom_kod, kommun) %>%
  summarise(sum_anslut_kom = sum(n_ladd_punkt), # lägg till na
            sum_station_kom = n())
...
Kartan är ett utkast på en interaktiv karta med lager som kan tändas och släckas med ikonen under zoom uppe till vänster i kartan. Det primära kartlagret är punkter på laddstationer(xy). De andra kartlagren är polygoner med antalet anslutningspunkter respektive laddstationer aggregerat till kommunnivå. Ett vänsterklick på ett objekt visar all tillgänglig information i den tabell som ligger till grund för aktuellt lager (ett vänsterklick utanför popupen i kartan stänger tabellen). När muspekaren vilar på ett objekt visas antal anslutningspunkter på aktuell laddstation eller summan av anslutningspunkter/laddstationer per kommun (beroende på vilket lager som är tänt).
...
mapview(laddst_anslut_kom, zcol = "sum_anslut_kom", col.regions = reds(laddst_anslut_kom$sum_anslut_kom), at = seq(0, 150, 30), alpha.regions = 0.2, layer.name = c("Anslutningspunkter"))+
  mapview(laddst_anslut_kom, zcol = "sum_station_kom", col.regions = orgs(laddst_anslut_kom$sum_station_kom), at = seq(0, 45, 15), alpha.regions = 0.2, layer.name = c("Laddstationer"))+
  mapview(laddstationer_punkt, zcol = "n_ladd_punkt", legend = FALSE, col.regions = pal(15), layer.name = c("Laddstationer (xy)"))
